...
export default function SkillDetail() {
    ...
    const fetchData = async () => {
        try {
            ...
            // Inject fallback weak points if missing
            if (!foundSkill.weakPoints || foundSkill.weakPoints.length === 0) {
                const lowerName = foundSkill.name.toLowerCase();
                if (lowerName.includes('react')) {
                    foundSkill.weakPoints = exampleWeakPoints.reactHooks;
                } else if (lowerName.includes('javascript') || lowerName.includes('js')) {
                    foundSkill.weakPoints = exampleWeakPoints.javaScript;
                } else if (lowerName.includes('html')) {
                    foundSkill.weakPoints = exampleWeakPoints.html;
                } else if (lowerName.includes('css')) {
                    foundSkill.weakPoints = exampleWeakPoints.css;
                }
            }
                            ...
                            {skill.resources.map((resource, i) => {
                                ...
                                return (
                                    <a
                                        ...
                                        className={`group flex items-center gap-4 p-4 rounded-xl border transition-all hover:scale-[1.01] hover:shadow-lg ${isYouTube
                                            ...
                                            <div key={qIndex} className="space-y-4">
                                                ...
                                                >
                                                    {q.options.map((option, oIndex) => (
                                                        <div
                                                            key={oIndex}
                                                            className={`flex items-center space-x-3 p-4 rounded-lg border transition-all ${quizSubmitted
